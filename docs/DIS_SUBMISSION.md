...
Selecting a card opens a 3D terrain with a path laid across it; each marker corresponds to a message (up to the first 30 messages are displayed by default, with support for longer conversations).
...
Selecting a card transitions to the 3D view. The 3D terrain uses an isometric-like perspective camera (approximately 30° elevation, 45° azimuth) for stable, consistent viewing. Participants can:
...
- **Switch camera views** between default perspective, side orthographic, and top orthographic views
- **Adjust camera distance, elevation, and rotation** via the settings panel
...
- **Manual Slider**: In the settings panel, provides fine-grained scrubbing control.
...
**Camera Controls**:
- **Preset Views**: Three camera modes (default perspective, side orthographic, top orthographic) accessible from the right panel
...
- Settings panel with camera and terrain controls
