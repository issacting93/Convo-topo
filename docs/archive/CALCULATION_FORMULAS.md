...
This document provides the complete mathematical foundation for calculating all 10 metadata dimensions using algorithmic/rule-based methods. Each formula, threshold, and decision rule is documented with examples.

**Note**: This approach differs from the current LLM-based classifier v1.1 which uses Claude API for classification with probability distributions for roles.
